...
export class RegisterModuleComponent implements OnInit {
  ...
  onFilter(data: any) {
    this.List = [];
    this.moduleList.filter((x) => {
      if (x.name.toLowerCase().includes(data.toLowerCase()) || x.path.toLowerCase().includes(data.toLowerCase())) this.List.push(x);
    });
  }
  ...
  onAddNew() {
    if (this.ModuleForm.valid) {
      ...
      if (!this.ModuleForm.get("module_id").value) {
        this.apiService.postTypeRequest("module/insert", this.ModuleForm.value).subscribe((result: any) => {
          if (result.result) {
            this.ngOnInit();
        ...
        this.apiService.postTypeRequest("module/update", this.ModuleForm.value).subscribe((result: any) => {
          if (result.result) {
            this.ngOnInit();
            this.toster.success("New Data Added");
            swalWithBootstrapButtons.fire("Added!", result.message, "success");
  ...
  async onDelete(id: string) {
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.value) {
          this.apiService
            .postTypeRequest("module/delete", Request_Data)
            .toPromise()
            .then((result: any) => {
              if (result.result) {
