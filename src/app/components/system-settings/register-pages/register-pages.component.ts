...
export class RegisterPagesComponent implements OnInit {
  ...
  onFilter(data:any){
    ...
    this.pageList.filter( (x) => {
      if(x.component_name.toLowerCase().includes(data.toLowerCase()) || x.route.toLowerCase().includes(data.toLowerCase()) || x.comdule_name.toLowerCase().includes(data.toLowerCase()))
      this.List.push(x)
    })
  }
