...
export class RegisterPagesComponent implements OnInit {
  ...
  constructor(public apiService: ApiService, private dialog: MatDialog, public spinner: SpinnerService, public toster: ToastrService) {}
  ...
  ngOnInit() {
    this.dataFetch = true;
    this.apiService.getTypeRequest("table_data/MODULE").subscribe((result: any) => {
      if (result.result) {
  ...
  onFilter(data: any) {
    ...
    this.pageList.filter((x) => {
      if (x.component_name.toLowerCase().includes(data.toLowerCase()) || x.route.toLowerCase().includes(data.toLowerCase()) || x.comdule_name.toLowerCase().includes(data.toLowerCase())) this.List.push(x);
    });
  }
  ...
  onAddNew() {
    if (this.ComponentForm.valid) {
      ...
      if (!this.ComponentForm.get("component_id").value) {
        this.apiService.postTypeRequest("component/insert", this.ComponentForm.value).subscribe((result: any) => {
          if (result.result) {
            this.ngOnInit();
        ...
        this.apiService.postTypeRequest("component/update", this.ComponentForm.value).subscribe((result: any) => {
          if (result.result) {
            this.ngOnInit();
            this.toster.success("New Data Added");
            swalWithBootstrapButtons.fire("Added!", result.message, "success");
  ...
  async onDelete(id: string) {
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.value) {
          this.apiService
            .postTypeRequest("component/delete", Request_Data)
            .toPromise()
            .then((result: any) => {
              if (result.result) {
