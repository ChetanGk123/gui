...
export class DocumentsComponent implements OnInit {
  ...
  deleteRecord(data){
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.value) {
          this.apiService
            .postTypeRequest("delete_student_document", Request_Data)
            .toPromise()
            .then((result: any) => {
              if (result.result) {
  ...
  async saveFile() {
    this.loader = true
    await this.apiService.postTypeRequest("save_document_data", this.fileGroup.value).subscribe(async (result: any) => {
      if (result.result) {
  ...
  async updateFile() {
    this.loader = true
    await this.apiService.postTypeRequest("update_student_document_data", this.fileGroup.value).subscribe(async (result: any) => {
      if (result.result) {
        this.toster.success("Data added successfully");
        this.ngOnInit();
        this.update = false;
